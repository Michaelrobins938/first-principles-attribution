...
class WhitePaperPDF:
    ...
    def add_title_page(self):
        ...
        subtitle_text = Paragraph('Technical Whitepaper v2.0.0', subtitle_style)
    ...
    def add_content_from_structure(self, structure):
        ...
        for item in structure:
            if item['type'] == 'title':
                ...
                p = Paragraph(cleaned_content, self.styles['Normal'])
                self.elements.append(p)
            elif item['type'] == 'bold':
                cleaned_content = clean_text_for_pdf(item['content'])
                p = Paragraph(f'<b>{cleaned_content}</b>', self.styles['Normal'])
                self.elements.append(p)
            elif item['type'] == 'table':
                # Create table
                ...
                cleaned_content = clean_text_for_pdf(item['content'])
                p = Paragraph(f'<pre>{cleaned_content}</pre>', self.code_style)
                self.elements.append(p)
            elif item['type'] == 'empty':
...
def main():
    ...
    pdf_output_path = r"D:\first-principles-attribution-repo\docs\FIRST_PRINCIPLES_ATTRIBUTION_WHITEPAPER_PROFESSIONAL_FORMAT.pdf"
    ...
    docx_writer = WhitePaperDOCX()
    docx_writer.add_title_page()
    docx_writer.add_content_from_structure(structure)
    docx_writer.save(docx_output_path)
